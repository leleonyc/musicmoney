...
import {
  ...
  ArrowRight, UserPlus, LogIn, Search, Target, CalendarClock, Phone, User,
} from "lucide-react";
...
export default function MusiCash() {
  ...
  const [loginForm, setLoginForm] = useState({ nome: "", telefone: "" });
  const [createForm, setCreateForm] = useState({ nome: "", telefone: "" });
  const [authError, setAuthError] = useState("");
  const [authBusy, setAuthBusy] = useState(false);
  const [authed, setAuthed] = useState(false);
  const [userId, setUserId] = useState(null); // id da linha do usuário no Supabase
  ...
  // ---- persistência: cada usuário é uma linha na tabela "users" do Supabase.
  // Ao logar/criar conta, já carregamos os dados direto para o estado (veja
  // submitLogin/confirmCreate). Daqui em diante, qualquer mudança em saldo,
  // avaliações, saques etc. é salva de volta nessa mesma linha (por "id"),
  // assim o progresso aparece em qualquer aparelho que fizer login. ----
  ...
  useEffect(() => {
    ...
    persist({ balance, ratings, withdrawals, dailyDate, dailyCount });
  }, [balance, ratings, withdrawals, dailyDate, dailyCount, authed, loaded, userId, persist]);
  ...
  // aplica os dados de uma linha do Supabase no estado do app e entra
  const applyLoadedUser = (row) => {
    ...
    setDailyDate(row.daily_date ?? today);
    setDailyCount(row.daily_date === today ? (row.daily_count ?? 0) : 0);
    setAuthed(true);
    setLoaded(true);
    setView("app");
  };
  ...
  const confirmCreate = async () => {
    ...
    const digits = createForm.telefone.replace(/\D/g, "");
    try {
      // impede duas contas com o mesmo telefone
      ...
      if (existing) {
        setAuthError("Já existe uma conta com esse telefone. Tente entrar em vez de criar uma nova.");
        setAuthBusy(false);
        setView("create");
        return;
      }
      ...
      applyLoadedUser(inserted);
    } catch (e) {
      console.error("Falha ao criar conta", e);
      setAuthError("Não deu para criar sua conta agora. Tenta de novo em instantes.");
      setView("create");
    } finally {
      setAuthBusy(false);
    }
  ...
  const submitLogin = async (e) => {
    e.preventDefault();
    setAuthError("");
    const digits = loginForm.telefone.replace(/\D/g, "");
    if (!loginForm.nome.trim() || digits.length < 10) {
      setAuthError("Preencha nome e telefone válidos (com DDD).");
      return;
    }
    setAuthBusy(true);
    try {
      const { data, error } = await supabase
        .from("users")
        .select("*")
        .ilike("name", loginForm.nome.trim())
        .eq("phone", digits)
        .maybeSingle();
      if (error) throw error;
      if (!data) {
        setAuthError("Nome ou telefone não encontrados. Confira os dados ou crie um novo acesso.");
        setAuthBusy(false);
        return;
      }
      applyLoadedUser(data);
    } catch (e) {
      console.error("Falha ao entrar", e);
      setAuthError("Não deu para conectar agora. Tenta de novo em instantes.");
    } finally {
      setAuthBusy(false);
    }
  };
  ...
  const handleLogout = () => {
    ...
    setLoaded(false);
    setUserId(null);
    setLoginForm({ nome: "", telefone: "" });
    setCreateForm({ nome: "", telefone: "" });
    setBalance(0);
    setRatings({});
    setWithdrawals([]);
    setCreatedAt(null);
  ...
  // =========================================================================
  // LANDING / LOGIN — primeira tela do app: entrar com nome + telefone
  // =========================================================================
  if (view === "landing") {
    return (
        ...
        <div className="relative w-full max-w-sm" style={{ zIndex: 1 }}>
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <p className="mc-muted mt-3 leading-relaxed">
              Entre com seu nome e telefone cadastrados para acessar seu saldo.
            </p>
          </div>

          <form onSubmit={submitLogin} className="mc-panel rounded-xl p-6 space-y-4">
            <div>
              <label className="mc-muted block text-xs font-semibold uppercase tracking-wide mb-2">Nome completo</label>
              <div className="relative">
                <User className="mc-muted absolute left-3 top-1/2 -translate-y-1/2" size={16} />
                <input
                  value={loginForm.nome}
                  onChange={(e) => setLoginForm((f) => ({ ...f, nome: e.target.value }))}
                  className="mc-input pl-10 pr-4 py-3 rounded-lg"
            ...
            <div>
              ...
              <div className="relative">
                <Phone className="mc-muted absolute left-3 top-1/2 -translate-y-1/2" size={16} />
                <input
                  type="tel"
                  inputMode="tel"
                  value={loginForm.telefone}
                  onChange={(e) => setLoginForm((f) => ({ ...f, telefone: formatPhone(e.target.value) }))}
                  className="mc-input pl-10 pr-4 py-3 rounded-lg"
